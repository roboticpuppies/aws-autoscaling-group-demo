...
| `terraform/modules/` | Two reusable modules: `shared-infra` (VPC) and `app` (ASG + target group + IAM + SSM) |
...
- Each app's ASG attaches to a **target group**; the TG's health check drives ASG self-healing. No ALB is provisioned — load-balancing is out of scope for now. Clients reach the app directly on each instance's public IP
...
- [Rebuilding the AMI](docs/rebuilding-the-ami.md) — runbook for baking a new AMI and rolling it out to existing ASGs
